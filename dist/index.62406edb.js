...
class App {
    ...
    _newWorkout(e) {
        ...
        if (type == "running") {
            ...
            if (!validation(distance, duration, cadence) || !allPositive(distance, duration, cadence)) return alert("Inputs must be positive numbers");
            //coords, distance, duration, cadence
            workout = new Running(coords, distance, duration, cadence);
        }
        if (type === "cycling") {
            const elevation = +inputElevation.value;
            if (!validation(distance, duration, elevation) || !allPositive(distance, duration, elevation)) return alert("Inputs must be valid numbers");
    ...
    _renderWorkoutPopup(workout) {
        ...
        .setPopupContent(`<span> ${workout.type === "running" ? "\uD83C\uDFC3\u200D\u2642\uFE0F" : "\uD83D\uDEB4\u200D\u2640\uFE0F"}</span> ${workout.description}`).openPopup();
    }
    ...
    _moveToPopup(e) {
        ...
        const workout = this.#workout.find((item)=>item.id === workoutTarget.dataset.id);
